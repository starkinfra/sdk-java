...
public class TestIssuingPurchase {
    ...
    public void testQueryInstallmentCount() throws Exception {
        ...
        IssuingPurchase purchaseWithInstallments = null;
        for (IssuingPurchase purchase : purchases) {
            if (purchase.installmentCount != null) {
                purchaseWithInstallments = purchase;
                break;
            }
        }
        if (purchaseWithInstallments == null) {
            return;
        }

        IssuingPurchase purchaseExpected = IssuingPurchase.get(purchaseWithInstallments.id);
        Assert.assertNotNull(purchaseExpected.id);
        Assert.assertNotNull(purchaseExpected.installmentCount);
        Assert.assertTrue(purchaseExpected.installmentCount > 0);
    }
